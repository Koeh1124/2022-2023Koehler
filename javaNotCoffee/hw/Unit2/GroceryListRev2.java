...
public class GroceryListRev2 {
     ...
     private static GroceryItem[] addItem(GroceryItem item, GroceryItem[] ogList){
        GroceryItem[] groceryList = new GroceryItem[ogList.length+1];
        for(int i = 0;i<ogList.length;i++){
            groceryList[i] = ogList[i];
        }
        groceryList[-1] = item;
        return groceryList;
     }
     private static GroceryItem[] findItem(String name, GroceryItem[] ogList){
        GroceryItem[] groceryList = new GroceryItem[ogList.length-1];
        return groceryList;
     }
